...
7장 "코드 뒤의 수학과 통계"(`book/ch07-math-stats.adoc`)가 본문에 적어 둔 값들을 수치 계산 라이브러리로 다시 계산해서 맞는지 확인하는 실습 코드다. [Apache Commons Math 3](https://commons.apache.org/proper/commons-math/) 하나로 선형대수, 분포, 회귀, 검정을 모두 다루며, Gradle로 빌드한다. 검산 항목은 250건이다.
...
- softmax와 교차 엔트로피를 함께 쓰면 손실을 로짓으로 미분한 값이 "예측 확률 - 정답"이다. 수치 미분으로 세 성분을 모두 확인한다.
- 행렬 곱 두 개를 이어 붙인 W2(W1 x)는 하나의 행렬 (W2 W1)을 곱한 것과 같다. 비선형 함수 없이는 층을 쌓아도 표현력이 늘지 않는 이유다.
- Adam의 편향 보정을 거친 첫 스텝의 갱신량은 기울기의 크기와 무관하게 학습률 × (기울기의 부호)다.
- 4장의 이름 데이터에서 글자 a 다음의 전이 확률(이름의 끝 0.32, n 0.14, r 0.10, l 0.09)은 `names.txt`를 직접 세어 확인한다.
