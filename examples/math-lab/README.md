...
"수학·통계 기초" 부록(`book/appendix-math-stats.adoc`)이 본문에 적어 둔 값들을 수치 계산 라이브러리로 다시 계산해서 맞는지 확인하는 실습 코드다. [Apache Commons Math 3](https://commons.apache.org/proper/commons-math/) 하나로 선형대수, 분포, 회귀, 검정을 모두 다루며, Gradle로 빌드한다. 검산 항목은 159건이다.
...
| 부록 절 | 파일 |
|---|---|
| 벡터와 행렬 | `LinearAlgebra.java` |
| 랭크와 주성분 분석 | `LinearAlgebra.java` |
| 판별 분석 | `LinearAlgebra.java` |
| 미분과 체인 룰 | `Calculus.java` |
| 평균, 분산, 정규화 | `Statistics.java` |
| 정규 분포 | `Statistics.java` |
| 공분산과 상관 | `Statistics.java` |
| k-평균 군집화 | `Statistics.java` |
| 확률 분포와 조건부 확률 | `Probability.java` |
| 로짓, softmax, temperature | `Probability.java` |
| 로그, 교차 엔트로피, 최대 우도 | `Probability.java` |
| 엔트로피와 perplexity | `Probability.java` |
| KL 발산 | `Probability.java` |
| 선형 회귀와 로지스틱 회귀 | `Models.java` |
| 마르코프 체인 | `Models.java` |
| 벨만 방정식 | `Models.java` |
| 스케일링 법칙 | `Models.java` |
| 검색의 통계 | `SearchStats.java` |
| t-검정과 분산 분석 | `SearchStats.java` |
