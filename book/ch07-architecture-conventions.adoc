...
텍스트를 토큰으로 나누는 규칙과 사전. BPE 병합 규칙, 특수 토큰 정의, 채팅 템플릿이 들어 있다. 어휘 크기는 모델에 따라 3만에서 25만 개 수준이다. microGPT의 27자 문자 사전과 역할이 같고, 규모와 알고리즘만 다르다.
...
1. *Safetensors*: 단순하고 안전한 바이너리 포맷. 6장에서 Go로 직접 만들어 본 것처럼 어느 언어에서든 다룰 수 있다.
2. *transformers의 config 규약*: `model_type`으로 아키텍처를 자동 식별한다.
3. *소수 아키텍처로의 수렴*: 추론 엔진들이 미리 구현해 둘 수 있을 만큼 아키텍처 종류가 적다.

셋 다 표준 기구의 스펙이 아니라 생태계가 수렴한 결과다. 다음 부에서는 이렇게 유통되는 모델을 실제 서비스에 붙일 때 가장 자주 만드는 시스템인 RAG를 다룬다.
