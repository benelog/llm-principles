...
이 호환성이 성립하는 이유는 표준 문서가 있어서가 아니라, 공개되는 모델 대부분이 소수의 아키텍처(Llama, Mistral, Qwen, Gemma 계열)를 채택하기 때문이다. 각 엔진은 이 소수의 아키텍처를 미리 구현해 두고, ``config.json``의 ``model_type``을 보고 해당 구현을 선택한다. 이것이 세 번째 표준인 "소수 아키텍처로의 수렴"이다.
